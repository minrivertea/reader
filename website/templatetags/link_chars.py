...
def link_chars(value):
    """Wraps Chinese characters in a link tag which will then look them up"""

    RE = build_re() 
            
    newtext = ''
    for x in smart_unicode(value):
        if RE.match(x):
...
register.filter('link_chars', link_chars)